...
class SanternoLogger:
    ...
    def __init__(self):
        if not path.exists(SanternoConfig.PORT_DEV):
            app_logging.error("Device port %s does not exist", SanternoConfig.PORT_DEV)
            return
    ...
    def __measure(self):
        ...
        while True:
            ...
            modbus_data = DevData(CsvConfig.SANTERNO_LOG_DIR_AND_FILE_PREFIX, JsonRequestType.SANTERNO)

            for inverter in self.inverters:
                dev_data_read = inverter.read_data_json()
                if dev_data_read is None:
                    app_logging.debug("Data [%s] could not be read.", inverter.dev_name)
                    continue
