...
class W1Ds18b20(BaseDevice):
    __MAX_INIT_ATTEMPTS = 10
    __RE_INIT_WAIT_TIME_SEC = 2
    __MAX_COUNT_MEASURES = 45
    __MAX_OBSOLETE_DATA_THRESH = 10
    ...
    def __try_initialize(self):
        ...
        for _ in range(self.__MAX_INIT_ATTEMPTS):
            try:
                self.w1_sensor = W1ThermSensor(Sensor.DS18B20, self.dev_id)
            ...
            # pylint: disable=W0703
                ...
                sleep(self.__RE_INIT_WAIT_TIME_SEC)
                app_logging.error(
                    "Sensor id=[%s] could not be initialized. Next attempt...",
                    self.dev_id,
                )
    ...
    def read_temperature(self):
        ...
        if len(self.temperatures) > self.__MAX_COUNT_MEASURES:
        ...
        # pylint: disable=W0703
            ...
            app_logging.debug(
                "Read temperature from sensor '%s' ('%s') failed.\n%s",
                self.dev_id,
                self.dev_name,
                ex,
            )
    ...
    def read_data_json(self) -> dict or None:
        ...
        if self.obsolete_data_count >= self.__MAX_OBSOLETE_DATA_THRESH:
            app_logging.debug("Obsolete data. Obsolete data counter: %d", self.obsolete_data_count)
            avg_temp = None

        return OrderedDict({"temperature": avg_temp})
