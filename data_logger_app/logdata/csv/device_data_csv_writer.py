...
class DeviceDataCsvWriter:
    ...
    def __get_merged_dict(log_date_time, dev_data, dev_names):
        ...
        data_to_zip = [f"{v:.2f}" if v else v for d in dev_data for v in d.values()]
    ...
    def __write_data_to_file(self, file_path, dev_data, write_header=False):
        ...
        try:
            with open(file_path, "a") as csv_file:
                ...
                self.csv_writer = DictWriter(csv_file, fieldnames=data_merged.keys(), delimiter=",")
                if write_header:
    ...
    def write_data(self, file_path, dev_data):
        self.__write_data_to_file(file_path, dev_data, write_header=not path.isfile(file_path))
